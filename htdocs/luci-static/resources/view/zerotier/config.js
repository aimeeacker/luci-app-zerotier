...
function renderStatus(isRunning) {
	var template = '<em><span style="color:%s"><strong>%s %s</strong></span></em>';
	return String.format(template, isRunning ? 'green' : 'red', _('ZeroTier'),
		...
		s.render = function() {
			poll.add(function() {
				return L.resolveDefault(getServiceStatus()).then(function(res) {
					...
					if (status)
						status.innerHTML = renderStatus(res);
				});
			});
